...
OUTFILENAME="AGL-Host-Configuration.pdf"

SCRIPT=$(basename "${BASH_SOURCE[@]}")

VERSION=$(grep '"version":' "$(dirname "${BASH_SOURCE[@]}")/book.json" | cut -d'"' -f 4)
[ "$VERSION" != "" ] && OUTFILENAME="${OUTFILENAME%.*}_v${VERSION}.pdf"
...
while [ $# -gt 0 ]; do
	case "$1" in
        ...
        -d|--dry)               DRY="echo";;
        -h|--help)              usage;;
        -o|--output)            shift; OUTFILENAME="$(basename $1)"; OUT_DIR="$(cd $(dirname $1) && pwd)";;
